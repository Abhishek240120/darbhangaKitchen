...
export default function MenuItem(props){
	...
	return (<div className="menu-toppicks">
					...
					arr.map((dish)=>
						<div class="card" style={{"width": "18rem"}}>
  								...
  								<div class="card-body">
    								...
    								<button value={dish.id} class="w3-button  w3-circle w3-teal" onClick={(event)=>{removeItem(event)}}>-</button>
  									<span>  </span><span style={{"font-size":"2em"}}>{orders[dish.id]}</span>
  									<span>  </span><button value={dish.id} class="w3-button  w3-circle w3-red w3-card-4" onClick={(event)=>{addItem(event)}}>+</button>
  								</div>
						</div>)
